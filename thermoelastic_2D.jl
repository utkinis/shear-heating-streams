...
@kernel function update_stress_and_fourier!(Pr, Txx, Tyy, Txy, Vx, Vy, qTx, qTy, T, dTdt_adv, K, G, lambda, dt, dx, dy)
...
@kernel function update_velocity_and_temperature!(Vx, Vy, Pr, Txx, Tyy, Txy, rho, T, qTx, qTy, Cp, dTdt_adv, dt, dx, dy)
    ix, iy = @index(Global, NTuple)
    ...
    @inbounds if ix <= size(T, 1) && iy <= size(T, 2)
        ...
        T[ix, iy] += dt / (rho[ix, iy] * Cp[ix, iy]) * (dTdt_adv[ix, iy] - divqT)
    end
end
