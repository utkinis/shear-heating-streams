...
@views function braeck_1D()
    ...
    limits!(axs.T, -2, 2, 0.5Tbg_E_R, 100Tmaxa)
    limits!(axs.Vy, -2, 2, -5e3, 5e3)
    limits!(axs.σ_evo, 0.0, ttot / τr, 0.0, 1.0)
    limits!(axs.Tmax_evo, 0.0, ttot / τr, 0.5T0_E_R, 100Tmaxa)
    ...
    while tcur < ttot
        ...
        for iter in 1:niter
            ...
            Vy[[1, end]] .= .-Vy[[2, end - 1]]
            # temperature
            fQx[2:end-1] .= -χ .* Diff(T[2:end-1]) ./ dx .- (1 / C) .* σxy[2:end-1] .* avx(Vy[2:end-1])
            T[2:end-1]   .= T_old[2:end-1] .+ dt .* (.-Diff(fQx) ./ dx)
            # fQx[2:end-1] .= -χ * diff(T[2:end-1]) ./ dx
            # T[2:end-1] .= T_old[2:end-1] .+ dt * (.-diff(fQx) ./ dx .+ (1 / C) .* avx(σxy .^ 2 ./ μ))
            T[[1, end]]  .= T[[2, end - 1]]
            if iter % ncheck == 0
